...
export function DataTableHeader<TData>({
  ...
  const sortableColumnIds = useMemo(() => {
    ...
    return table
      .getAllLeafColumns()
      .filter((column) => !NON_REORDERABLE_COLUMNS.projects.has(column.id))
        ...
        >
          ...
          >
            {headerGroup.headers.map((header, headerIndex, headers) => {
              ...
              const canReorder = !NON_REORDERABLE_COLUMNS.projects.has(
                columnId,
              );
...
function renderHeaderContent<TData>(
  ...
  const sortField = SORT_FIELD_MAPS.projects[columnId];
