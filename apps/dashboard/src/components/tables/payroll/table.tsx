...
type PayrollDataTableProps = {
  entries: PayrollEntryTableRow[];
  entryCount: number;
  fetchNextPage: () => void;
  hasNextPage: boolean;
...
export function PayrollDataTable({
  ...
  const { setColumns } = usePayrollStore();
  ...
  const tableScroll = useTableScroll({
    startFromColumn: STICKY_COLUMNS.payroll.length,
    useColumnWidths: true,
  });
  const rows = table.getRowModel().rows;
  const rowHeight = ROW_HEIGHTS.payroll;
  ...
  return (
    <>
      <PayrollTableHeader entryCount={entryCount} />
