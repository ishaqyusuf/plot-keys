...
export function DataTableHeader<TData>({
  ...
  const sortableColumnIds = useMemo(() => {
    ...
    return table
      .getAllLeafColumns()
      .filter((column) => !NON_REORDERABLE_COLUMNS.appointments.has(column.id))
        ...
        >
          ...
          >
            {headerGroup.headers.map((header, headerIndex, headers) => {
              ...
              const canReorder =
                !NON_REORDERABLE_COLUMNS.appointments.has(columnId);
...
function renderHeaderContent<TData>(
  ...
  const sortField = SORT_FIELD_MAPS.appointments[columnId];
