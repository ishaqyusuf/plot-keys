...
import { columns, type AgentTableRow } from "./columns";
import { DataTableHeader } from "./data-table-header";
import { AgentsTableHeader } from "./table-header";

const NON_CLICKABLE_COLUMNS = new Set(["actions"]);

type AgentsDataTableProps = {
  agentCount: number;
...
export function AgentsDataTable({
  ...
  const { setColumns } = useAgentsStore();
  ...
  const tableScroll = useTableScroll({
    startFromColumn: STICKY_COLUMNS.agents.length,
    useColumnWidths: true,
  });
  const rows = table.getRowModel().rows;
  const rowHeight = ROW_HEIGHTS.agents;
  ...
  return (
    <>
      <AgentsTableHeader agentCount={agentCount} />
