...
export function TableSkeleton<TData>({
  ...
  const stickyColumns = useMemo(() => {
    return stickyColumnIds.map((id) => {
      const column = orderedColumns.find((col) => getColumnId(col) === id);

      return {
        id,
        width: columnSizing[id] ?? column?.size ?? 150,
      };
    });
  }, [orderedColumns, stickyColumnIds, columnSizing]);
