...
export async function createProjectAction(formData: FormData) {
  ...
  const type =
    (String(formData.get("type") ?? "").trim() as
      | "building"
      | "estate"
      | "fit_out"
      | "infrastructure"
  ...
  const startDateRaw = String(formData.get("startDate") ?? "").trim();
  const targetDateRaw = String(formData.get("targetCompletionDate") ?? "").trim();

  if (!name) {
    redirect(
      createRedirectUrl("/projects", { error: "Project name is required." }),
    );
  }
  ...
  await prisma.project.create({
    data: {
      ...
      description,
      startDate: startDateRaw ? new Date(startDateRaw) : null,
      targetCompletionDate: targetDateRaw ? new Date(targetDateRaw) : null,
    },
  });
...
export async function createProjectPhaseAction(formData: FormData) {
  ...
  if (!name) {
    redirect(
      createRedirectUrl(`/projects/${projectId}`, {
...
export async function createProjectUpdateAction(formData: FormData) {
  ...
  const projectId = String(formData.get("projectId") ?? "");
  const kind =
    (String(formData.get("kind") ?? "").trim() as
      | "daily"
      | "weekly"
      | "milestone"
      | "general") || "general";
...
export async function createProjectIssueAction(formData: FormData) {
  ...
  const description =
    String(formData.get("description") ?? "").trim() || null;
  const severity =
    (String(formData.get("severity") ?? "").trim() as
      | "low"
      | "medium"
      | "high"
      | "critical") || "medium";
...
export async function assignProjectMemberAction(formData: FormData) {
  ...
  const membershipId = String(formData.get("membershipId") ?? "");
  const projectRole =
    (String(formData.get("projectRole") ?? "").trim() as
      | "project_owner"
      | "project_manager"
      | "qs_manager"
      | "finance_reviewer"
  ...
  await prisma.projectAssignment.upsert({
    ...
    update: { projectRole, status: "active" },
    create: { projectId, membershipId, projectRole },
  });
