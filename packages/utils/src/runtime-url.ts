...
export function stripPortFromRuntimeHost(host: string) {
...
function hostMatchesRoot(host: string, root: string) {
  const hostCandidates = [host, stripPortFromRuntimeHost(host)];
  const rootCandidates = [root, stripPortFromRuntimeHost(root)];
...
function isIpHost(host: string) {
  const bareHost = stripPortFromRuntimeHost(host).replace(/^\[|\]$/g, "");
...
function isLocalhostHost(host: string) {
  const bareHost = stripPortFromRuntimeHost(host);
  return bareHost === "localhost" || bareHost.endsWith(".localhost");
}

function isVercelPreviewHost(host: string) {
  return stripPortFromRuntimeHost(host).endsWith(".vercel.app");
}
