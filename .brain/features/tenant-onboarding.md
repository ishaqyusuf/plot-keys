...
- `apps/dashboard` now includes working routes for:
  ...
  - `/builder`
  - `/live`
- Auth entry pages use client-side `use-zod-form` forms backed by tRPC auth mutations.
- Onboarding state, step progress, recommendations, profile refresh/editing,
  and completion use the dedicated `onboarding` tRPC router. Builder mutations
  use their API-owned contracts rather than dashboard server actions.
...
- The current onboarding implementation captures all five guided input groups,
  persists resumable progress, derives a tenant profile, recommends templates,
  and creates the initial personalized website bundle.
