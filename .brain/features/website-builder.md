...
- `apps/dashboard/src/app/builder/page.tsx` now provides:
  ...
  - publish action
- `apps/dashboard/src/app/(app)/page.tsx` owns the tenant overview and routes
  website work into the dedicated full-bleed builder.
...
- Dedicated Website API mutations enforce template access based on the tenant
  company's stored subscription tier and licenses rather than relying on UI
  state alone.
- Builder reads, draft creation, content/theme editing, AI generation, and
  publishing use `website.*`; template discovery uses `templates.catalog`.
