...
- API ownership is split by domain: `website.*` owns tenant website state and
  mutations, `templates.*` owns catalog/licenses, and `stockImages.*` owns
  stock-image entitlement and purchase.
- Subscription state changes remain provider-verified under Billing/webhook
  ownership; the former manual generic-router mutation was removed.
...
- The broader marketplace, version graph, logo system, and unified billing
  center remain incomplete. A basic stock-image catalog, tenant license, and
  billing-line purchase path are implemented.
