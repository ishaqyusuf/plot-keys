...
- Richer section configs may evolve from flat string fields toward content-node objects with:
  ...
  - optional `maxLength`
  - optional `aiEnabled`
# QA company lifecycle

- `Company` stores QA classification, source domain, marked timestamp, and
  purge-start timestamp.
- Global `QaPurgeRun` stores actor, timestamps, status, aggregate deletion/file
  counts, and error category only.
