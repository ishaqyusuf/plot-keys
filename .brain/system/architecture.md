...
- `apps/sandbox`: Platform-admin template testing, sandbox profile management,
  and public read-only share previews
...
- `packages/website-builder`: Reusable template preview presentation/runtime
  components and sandbox render-data normalization. App-owned routing,
  authentication, and mutation adapters do not belong in this package.
...
- Sandbox share previews are not tenant-site pages. They are owned by
  `apps/sandbox` at `/preview/[shareId]/[[...slug]]`, are public/read-only,
  preserve draft/live mode through internal navigation, and are always
  `noindex`.
...
- `apps/sandbox` is the sole runtime owner of template sandbox authoring and
  preview routes. It mounts the focused `SandboxAppRouter`, uses a host-scoped
  platform session, and rejects authenticated non-platform-admin authors.
- Sandbox profile persistence remains in the shared
  `template_sandbox_profiles` table under the
  `template-sandbox-public` service owner. Sandbox operations never create
  tenants, domains, websites, or published production records.
- Dashboard and tenant-site sandbox URLs are compatibility redirects only.
  Shared rendering/presentation code lives in `packages/website-builder`;
  dashboard and sandbox tRPC/routing adapters remain app-local.
