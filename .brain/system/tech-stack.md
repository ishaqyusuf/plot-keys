...
- Local named-host dev: Portless-capable workspace scripts. Website/dashboard QA should use `bun run dev --local --filter dashboard tenant-site`, then browse `<tenant>.tenant-plotkeys.localhost` for public tenant sites and `dashboard.<tenant>.app-plotkeys.localhost` for tenant dashboard flows.
