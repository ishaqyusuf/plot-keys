...
## Applications
- `apps/dashboard`: Implemented Next.js tenant dashboard with working sign-up, sign-in, verify-email, onboarding, builder, and live-preview flows; auth entry now uses a Midday-style `src/trpc/client.tsx` and `src/trpc/server.tsx` split, a dashboard-owned `/api/trpc` route, `use-zod-form`, and a temporary dashboard session bridge route while onboarding/builder/domain mutations still include legacy server-action paths during the ongoing migration; onboarding is now a 6-step URL-driven multi-step flow (business identity → market focus → brand style → contact & operations → content readiness → template launch) with per-step DB persistence and a visual progress checklist, step 06 now creates the workspace and opens the template configuration screen so the tenant can edit website look/text before using the builder publish button, sign-up now hands off immediately to the tenant-scoped onboarding host for verification and setup, sign-in is blocked on the shared app host, dashboard session membership now resolves against the current tenant host to prevent cross-tenant access, and dev-only dashboard forms now follow an explicit profile-based quick-fill system that new forms are expected to wire in at build time; agent/employee onboarding is now invite-driven so invited users land on `/join/[token]`, authenticate, accept the invite, and complete their own profile details directly on the site.
...
- `apps/tenant-site`: Implemented Next.js tenant website renderer that now loads published tenant site configurations from Prisma, can resolve by tenant hostname, falls back to sample content when no live tenant site exists, and includes the first real customer-portal auth slice under `/portal/*` with tenant-scoped Better Auth session cookies, guarded portal routes, and customer-session-aware dashboard/account pages.
...
- `packages/auth`: Implemented route helpers with Better Auth session verification and Prisma adapter wiring; `packages/auth/src/better-auth.ts` configures the Better Auth instance with the Prisma adapter, email/password plugin, and the `plotkeys` cookie prefix; `createBetterAuthSession` creates DB-backed sessions and `getAppSessionFromBetterAuth` resolves sessions from request headers via `auth.api.getSession()`; browser-safe constants live behind a shared subpath so client bundles do not import database-backed auth code.
...
- `packages/db`: Implemented provider-aware shared database package with Prisma-owned schema/migrations, Prisma Client, a growing `src/queries` layer for reusable Prisma-backed domain queries, and a mirrored Drizzle query layer; current implemented Prisma models cover `users`, `companies`, `memberships`, `site_configurations`, `tenant_domains`, `properties`, `agents`, `tenant_onboardings` (with full onboarding step fields for business identity, market focus, brand style, contact operations, and content readiness), and the Better Auth tables (`auth_sessions`, `auth_accounts`, `auth_verifications`).
- `packages/email`: Implemented a Midday-aligned React Email package structure with shared email defaults, starter email components, a render helper, and email templates for welcome, verification, new-lead, site-published, and workspace-invitation flows.
- `packages/jobs`: Implemented job utilities including `runWithRetry` and `runInBackground` queue helpers with exponential-backoff retry logic, and a `domainSyncHandler` that processes Vercel tenant-domain provisioning per company with per-domain error isolation.
- `packages/notifications`: Implemented framework-agnostic notification types with separate file-based definitions under `src/types/`, a typed notification-definition registry, recipient contacts for users/subscribers, an in-memory notification store contract using `notificationType` naming, a GND-style `payload-utils` trigger layer, and service classes for notification triggering plus real Resend/WhatsApp delivery wiring; current notification types now include workspace invitation emails used by team, agent, and employee invite flows.
...
## Important Files
- `package.json`: Implemented monorepo workspace manifest and root dev router. Use `bun run dev` for the full Portless stack and `bun run dev -f <package>` for scoped Turbo filters on named `.localhost` URLs.
...
- `apps/dashboard/DESIGN_SYSTEM.md`: Documents dashboard-specific product UI design rules and form/layout expectations.
- `apps/website/DESIGN_SYSTEM.md`: Documents marketing-site visual direction, composition rules, and brand usage guidance.
- `apps/tenant-site/DESIGN_SYSTEM.md`: Documents template-driven tenant-site theming and renderer design constraints.
...
- Domain/runtime note: tenant website and dashboard hostnames are stored as explicit Prisma `TenantDomain` records, the dashboard can sync them against Vercel as separate public/dashboard registrations, the dashboard runtime now accepts only dashboard-prefixed tenant hosts, and both dashboard and public renderers can resolve tenant context by hostname when records exist.
